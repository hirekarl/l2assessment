...
describe('useDashboardStats', () => {
  ...
  it('counts fallback-mode entries separately from llm entries', () => {
    seedHistory([
      { category: 'Billing Issue', urgency: 'Low', timestamp: new Date().toISOString(), source: 'mock' },
      { category: 'Billing Issue', urgency: 'Low', timestamp: new Date().toISOString(), source: 'llm' },
      { category: 'Billing Issue', urgency: 'Low', timestamp: new Date().toISOString() }
    ])
    const { result } = renderHook(() => useDashboardStats())
